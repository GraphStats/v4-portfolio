...
export function AdminFeedbackList({ initialFeedbacks }: AdminFeedbackListProps) {
  ...
  const handleToggleStatus = async (item: Feedback) => {
    ...
    try {
      ...
      toast.success(nextIsCorrected ? "Feedback marque comme corrige" : "Feedback repasse en nouveau")
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="v4-card rounded-[2rem] border-white/5 p-5 flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
        <div className="flex items-center gap-2 text-sm">
          ...
          <Badge className="bg-primary/20 text-primary hover:bg-primary/20">{countNew} nouveau(x)</Badge>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant={statusFilter === "all" ? "default" : "outline"} onClick={() => setStatusFilter("all")}>Tous</Button>
            <Button size="sm" variant={statusFilter === "new" ? "default" : "outline"} onClick={() => setStatusFilter("new")}>Nouveaux</Button>
            <Button size="sm" variant={statusFilter === "corrected" ? "default" : "outline"} onClick={() => setStatusFilter("corrected")}>Corriges</Button>
          </div>
        </div>
      </div>
              ...
              <article key={item.id} className="v4-card rounded-[2rem] border-white/5 p-6 space-y-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{item.email || "Aucun email"}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isCorrected ? (
                      <Badge className="bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20">Corrige</Badge>
                    ) : (
                      <Badge className="bg-amber-500/20 text-amber-300 hover:bg-amber-500/20">Nouveau</Badge>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-3 text-sm">
                  <div>
                    <p className="text-xs uppercase tracking-widest text-primary/80 mb-1">Points positifs</p>
                    <p className="text-foreground/90 whitespace-pre-wrap">{item.positive_points}</p>
                  </div>

                  <div>
                    <p className="text-xs uppercase tracking-widest text-primary/80 mb-1">A ameliorer</p>
                    <p className="text-foreground/90 whitespace-pre-wrap">{item.negative_points}</p>
                  </div>

                  {item.additional_comment ? (
                    <div>
                      <p className="text-xs uppercase tracking-widest text-primary/80 mb-1">Commentaire</p>
